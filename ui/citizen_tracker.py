...
def run():
    ...
    def fetch_backend_data():
        try:
            amb_resp = requests.get(f"{BACKEND_URL}/ambulances/active", timeout=2).json()
            ...
            ambulances = amb_resp.get('ambulances', []) if isinstance(amb_resp, dict) else amb_resp
            incidents = inc_resp.get('incidents', []) if isinstance(inc_resp, dict) else inc_resp
            hospitals = hosp_resp.get('hospitals', []) if isinstance(hosp_resp, dict) else hosp_resp
            ...
            return (
                [
                    ...
                    {'id': 'ALS-002', 'latitude': 19.0860, 'longitude': 72.8877, 'status': 'responding', 'type': 'ALS', 'driver_name': 'Priya Singh'},
                    {'id': 'BLS-001', 'latitude': 19.0920, 'longitude': 72.8900, 'status': 'available', 'type': 'BLS', 'driver_name': 'Amit Patel'},
                    {'id': 'BLS-002', 'latitude': 19.0820, 'longitude': 72.8650, 'status': 'on_scene', 'type': 'BLS', 'driver_name': 'Suresh Nair'},
                    {'id': 'MINI-001', 'latitude': 19.0950, 'longitude': 72.8750, 'status': 'available', 'type': 'BIKE', 'driver_name': 'Deepa Gupta'},
                ],
                [
                    {'id': 'INC-001', 'latitude': 19.0800, 'longitude': 72.8800, 'severity': 'critical', 'incident_type': 'Cardiac', 'patient_name': 'Ramesh S.'},
                    {'id': 'INC-002', 'latitude': 19.0900, 'longitude': 72.8900, 'severity': 'severe', 'incident_type': 'Trauma', 'patient_name': 'Priya V.'},
                ],
                [
                    {'id': 'HOSP-001', 'name': 'Fortis Hospital', 'latitude': 19.0760, 'longitude': 72.8777, 'available_beds': 45, 'total_beds': 150},
                    {'id': 'HOSP-002', 'name': 'Apollo Clinic', 'latitude': 19.0860, 'longitude': 72.8877, 'available_beds': 78, 'total_beds': 200},
                    {'id': 'HOSP-003', 'name': 'Sai Nursing Home', 'latitude': 19.0900, 'longitude': 72.8950, 'available_beds': 22, 'total_beds': 80},
                ],
                False
            )
    ...
    with st.sidebar:
        # Live badge
        if is_live:
            st.markdown('<span class="live-badge live-green">● LIVE</span>', unsafe_allow_html=True)
        else:
            st.markdown('<span class="live-badge live-yellow">● DEMO</span>', unsafe_allow_html=True)

        st.markdown("### 📍 Your Location")
        st.session_state.user_lat = st.number_input("Latitude", value=st.session_state.user_lat, step=0.001, format="%.4f", key="ct_lat")
        st.session_state.user_lon = st.number_input("Longitude", value=st.session_state.user_lon, step=0.001, format="%.4f", key="ct_lon")
    ...
    with st.sidebar:
        ...
        if nearest_amb:
            ...
            type_colors = {'ALS': '#e74c3c', 'BLS': '#f39c12', 'BIKE': '#2ecc71', 'MINI': '#2ecc71'}
        ...
        incident_type = st.selectbox("Type", ["Cardiac", "Trauma", "Respiratory", "Burn", "Other"], key="ct_inc_type")
        severity = st.select_slider("Severity", ["Minor", "Moderate", "Severe", "Critical"], value="Severe", key="ct_sev")

        if st.button("🚑 REQUEST AMBULANCE", use_container_width=True, type="primary", key="ct_req"):
            try:
                payload = {
                ...
                if resp.status_code == 200:
                    ...
                    st.info(f"🚑 {data.get('ambulance_id', 'N/A')} dispatched! ({data.get('ambulance_type', 'BLS')})")
            ...
            if 'active_dispatch' not in st.session_state:
                ...
                st.info(f"⏱️ ETA: ~{max(1, int(nearest_amb['distance_km'] * 3)) if nearest_amb else '?'} min")
    ...
    with map_col:
        ...
        folium.CircleMarker(
            [user_lat, user_lon], radius=10,
            color='#3498db', fill=True, fillColor='#3498db',
            fillOpacity=0.8, weight=3,
            tooltip="📍 Your Location"
        ).add_to(m)
        # Outer pulse ring
        folium.CircleMarker(
            [user_lat, user_lon], radius=25,
            color='#3498db', fill=True, fillColor='#3498db',
        ...
        if 'active_dispatch' in st.session_state and 'route_coords' in st.session_state.active_dispatch:
            route_pts = st.session_state.active_dispatch['route_coords']
            if len(route_pts) > 1:
                folium.PolyLine(
                    route_pts,
                    color='#00ff00', weight=4, opacity=0.8,
                    tooltip="A* Optimized Route"
                ).add_to(m)
        elif nearest_amb:
            # Fallback direct line
            folium.PolyLine(
                [[user_lat, user_lon], [nearest_amb['latitude'], nearest_amb['longitude']]],
                color='#00d4ff', weight=2, opacity=0.6, dash_array='10',
    ...
    with info_col:
        ...
        for idx, amb in enumerate(ambulances_sorted[:5]):
            ...
            type_icon = {'ALS': '🔴', 'BLS': '🟠', 'BIKE': '🟢', 'MINI': '🟡'}.get(amb_type, '⚪')
